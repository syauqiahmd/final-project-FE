...
export default function Card(props) {
  ...
  const delFav = async () => {
    try {
      ...
      dispatch(fetchProjectByFavorite({userid: props.data.UserId, access_token: localStorage.access_token}))
  ...
  return (
    <div
      ...
      {location.pathname === "/favorite-project" ? (
        <div className="card-body" style={{display:"flex", alignItems:'end'}}>
          <button className="btn btn-outline-danger w-100" onClick={delFav}>Delete</button>
        </div>
      ) : (
